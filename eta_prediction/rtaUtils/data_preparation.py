...
def sample_data(dataframe: pd.DataFrame, sampling: int) -> pd.DataFrame:
...
def get_windows(dataframe: pd.DataFrame, window_size: int, encoders, scaler, features:dict) -> tf.data.Dataset:
    ...
    dataset = dataset.flat_map(lambda x: x.window(window_size, shift=1, stride=1, drop_remainder=True))
    dataset = dataset.flat_map(lambda window: window.batch(window_size))

    return dataset


def get_windows_at_time(dataframe: pd.DataFrame, time: int, window_size: int,
                        encoders, scaler, features:dict) -> tf.data.Dataset:
